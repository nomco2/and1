...
import com.example.heavyautometer.igeoscanpreapp.R;
...
public class CameraDialog extends DialogFragment {
	 ...
	 * @return
	 */
	public static CameraDialog showDialog(final Activity parent/*
																 * add
																 * parameters
																 * here if you
																 * need
	...
	public void onAttach(final Activity activity) {
		super.onAttach((com.example.heavyautometer.igeoscanpreapp.Preview) activity);
		if (mUSBMonitor == null)
			try {
				mUSBMonitor = ((com.example.heavyautometer.igeoscanpreapp.Preview) activity)
						.getUSBMonitor();
		...
		if (mUSBMonitor == null) {
			throw new ClassCastException(activity.toString()
					+ " must implement #getUSBController");
		}
	}
	...
	private final View initView() {
		final View rootView = getActivity().getLayoutInflater().inflate(
	...
	public void onResume() {
		super.onResume();
		updateDevices();
		final Button button = (Button) getDialog().findViewById(
	...
	public void updateDevices() {
		...
		final List<DeviceFilter> filter = DeviceFilter.getDeviceFilters(
				getActivity(), R.xml.device_filter);
	...
	private static final class DeviceListAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView,
			...
			if (convertView == null) {
				convertView = mInflater.inflate(R.layout.listitem_device,
						parent, false);
			}
			if (convertView instanceof CheckedTextView) {
				final UsbDevice device = getItem(position);
				((CheckedTextView) convertView).setText(String.format(
						"UVC Camera:(%x:%x)", device.getVendorId(),
						device.getProductId()));
			}
			return convertView;
		}
